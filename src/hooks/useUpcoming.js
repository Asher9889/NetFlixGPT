...
import { addUpcomingMovies } from "../utils/store/upcomingSlice";
...
const useNowPlaying = () => {
  ...
  async function fetchAllMovies() {
    try {
      const res = await fetch(IMDB_BASE_API_URL + "/movie/upcoming", options);
  ...
  async function fetchAllMoviesVideos() {
    try {
      ...
      if (allMovieData) {
        ...
        dispatch(addUpcomingMovies(finalData));
